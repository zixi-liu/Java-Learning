...
当我们需要某个类所有对象都共享一个变量时，考虑使用类变量。类变量的生命周期随类的加载开始，随着类的消亡而销毁。
...
main方法是虚拟机调用的。访问权限必须是public，不必创建对象，所以方法必须是static。
- 静态方法可以访问本类的静态成员。不可以访问本类的非静态成员。
...
构造器前面隐含了super()和调用普通代码块。
- 执行父类静态代码块和静态属性->子类静态代码块和静态属性->执行父类普块普通属性->父类构造方法->子类普块普通属性->子类构造方法

### 392 设计模式-单例模式

设计模式时在大量的实践中总结和理论化之后优选的代码结构。
...
- 抽象类不一定要包含abstract方法；
- 抽象方法不能有主体；
- 抽象类的子类必须全部重写父类的抽象方法：@override。因此抽象方法不能用private，final和static修饰；

### 402 接口

接口是给出一些没有实现的方法，封装到一起，到某个类要使用的时候，再根据具体方法写出来。
- interface 接口名{属性；方法（1.抽象方法；2.默认实现方法；3.静态方法；）}
- class 类名 implements 接口{自己属性；自己方法；必须实现的接口的抽象方法};
- 在接口中，抽象方法可以省略abstract。
- 在jdk8后，可以有默认实现方法，需要使用default修饰。也可以有静态方法。
